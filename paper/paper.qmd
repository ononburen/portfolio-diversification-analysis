...
where:
  ...
  $\beta_2$ captures the effect of extreme market volatility on portfolio returns,
  $\beta_3$ captures the interaction effect between HHI and market volatility, and
  $\epsilon$ is the error term

The linear regression model was implemented in R [@citeR], with results processed and cleaned by the packages `tidyverse` [@citeTidyverse] and `broom` [@citeBroom] with which `kable` [@citeKnitr] was used to visualize tables.
...
Furthermore, there is also the consideration that the HHI index involves theoretical measures of diversity that is impossible to achieve. While total concentration in a single asset (HHI = 1) can be achieved, our principle of absolute diversification (HHI=0) is impossible to acquire as this would instill an assumption of infinite assets with infinitesimally small weights in each of them.
